...
option_list <- list(
  ...
  make_option(c("-T", "--title"), type="character", default=NULL,
              help="Title of figure [default %default]",
              dest="Title")
)
...
if(opt$v){
  ...
  cat(paste0("Title (-T): ", opt$Title,"\n"))
}
opt$output_filename = unlist(strsplit(opt$output_filename, "/"))[length(unlist(strsplit(opt$output_filename, "/")))]

# read in alignments
alignments = read.table(opt$input_filename, stringsAsFactors = F, fill = T, row.names = NULL, quote = "", comment.char = "")
...
alignments$queryLen   <- as.numeric(alignments$queryLen)
alignments$queryStart <- as.numeric(alignments$queryStart)
alignments$queryEnd   <- as.numeric(alignments$queryEnd)
alignments$refLen     <- as.numeric(alignments$refLen)
alignments$refStart   <- as.numeric(alignments$refStart)
alignments$refEnd     <- as.numeric(alignments$refEnd)
alignments$lenAln     <- as.numeric(alignments$lenAln)
...
###############################################################################
## NOVA LÓGICA DE ORDENAÇÃO: Por número do Scaffold no eixo Y
###############################################################################

# Função auxiliar para extrair o número do scaffold da string
# Espera formato: "1::scaffold38:52487-77079" -> extrai 38
get_scaffold_number <- function(x) {
  # Procura por "scaffold" seguido de digitos
  match <- regexpr("scaffold([0-9]+)", x, perl = TRUE)
  if (match[1] > -1) {
    # Extrai a string correspondente
    str_match <- regmatches(x, match)
    # Remove a palavra 'scaffold' e converte para numérico
    return(as.numeric(sub("scaffold", "", str_match)))
  } else {
    return(Inf) # Se não achar 'scaffold+numero', joga para o fim da lista
  }
}

# 1. Identificar queries únicas
unique_queries <- unique(as.character(alignments$queryID))

# 2. Extrair os números para ordenação
scaffold_nums <- sapply(unique_queries, get_scaffold_number)

# 3. Criar a lista ordenada (ordem numérica, usa nome alfabético como desempate)
sorted_levels <- unique_queries[order(scaffold_nums, unique_queries)]

# 4. Aplicar a nova ordem ao fator queryID
alignments$queryID <- factor(alignments$queryID, levels = sorted_levels)
...
if(length(levels(alignments$refID)) > 1){
    # Calculo da media de identidade
...
# Função para limpar labels do Eixo Y (Mostra apenas scaffold38 ao inves da string completa)
clean_y_labels <- function(labels) {
    sapply(labels, function(x) {
        if(grepl("scaffold[0-9]+", x)) {
            return(sub(".*(scaffold[0-9]+).*", "\\1", x))
        } else {
            return(substr(x, 1, 20)) # fallback original
        }
    })
}
...
if (opt$similarity) {
  gp = ggplot(alignments) +
    ...
    theme(
      ...
      axis.text.x = element_text(size = 10, angle = 90, margin = margin(t = 10), hjust=1, vjust=0.5),
      axis.text.y = element_text(size = 10, angle = 0, margin = margin(t = 10), hjust=1, vjust=0.5)
    ) +
    # AQUI: Labels limpos no eixo Y
    scale_y_continuous(breaks = yTickMarks, 
                       labels = clean_y_labels(levels(alignments$queryID)),
                       guide = guide_axis(check.overlap = TRUE)) +
  ...
  gp = ggplot(alignments) +
    ...
    theme(
      ...
      panel.grid.minor.x = element_blank(),
      #panel.grid.major.x = element_blank(),
      axis.text.x = element_text(size = 10, angle = 90, margin = margin(t = 10), hjust=1),
      axis.text.y = element_text(size = 10, angle = 0, margin = margin(t = 10), hjust=1, vjust=0.5)
    ) +
    # AQUI: Labels limpos no eixo Y
    scale_y_continuous(breaks = yTickMarks, 
